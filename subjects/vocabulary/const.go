package vocabulary

const VocabularyTable string = "vocabularies"
const ContextSentencesTable string = "context_sentences"
const PronunciationAudiosTable string = "pronunciations"
const PartsOfSpeechTable string = "parts_of_speech"

const ContextSentencesID string = "vocabulary_id"
const PronunciationAudiosId string = "vocabulary_id"
const PartsOfSpeechId string = "vocabulary_id"
const vacabularyCharactersRow string = "characters"
